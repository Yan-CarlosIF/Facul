...
Params
    ...
    void (operation)(void*) = callback operation that will be applied to the
intere tree
*/
void tree_map(Node* root, void(operation)(void*)) {
  ...
  tree_map(root->lst,
           operation);    // do the operation only on the left side of the root
...
/*FILTER ELEMENTS IN THE TREE CREATING A NEW TREE
 ...
 Params
    root: pointer to the root of the tree
    condition: callback function that returns true if the element satisfies the
 condition copy_node: callback function that returns a copy of the element
    compare: callback function that compares two elements
...
void write_inorder(Node* new_node, FILE* fp, char* (*create_line)(void*)) {
  ...
  char* line = create_line(
      new_node->info);  // Generate a formatted string from the node's info
  if (line != NULL) {
...
/*LOAD A BINARY TREE FROM A TEXT FILE
    Reads a text file line by line, converts each line into data (using
read_line), creates a node for each data item, and inserts it into the tree in
order using the compare function.
...
*/
Node* tree_load_from_file(char* file_name, void*(read_line)(char*),
                          int(compare)(void*, void*)) {
