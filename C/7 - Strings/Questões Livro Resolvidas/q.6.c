...
combinação das palavras das duas frases recebidas. Exemplo: Frase 1: Hoje está
um belo dia Frase 2: Talvez chova amanhã Saída: Hoje talvez está chova um amanhã
belo dia
*/
...
void fraseGerada(char *str1, char *str2, char *str3) {
  ...
  int contPalavras1 = 0, j = 0, k = 0;

  char *token = strtok(str1, " \n");
  while (token != NULL) {
    palavras1[contPalavras1++] = token;
    token = strtok(NULL, " \n");
  }
